...
class CartResourceTest : AbstractTests() {
    ...
    fun `should return cart info with links`() {
        `when`(usersFeignClient.findBy(any())).thenReturn(UsersFixture.adaLovelace)
        `when`(productsProvider.findBy(any(), anyString())).thenReturn(ProductsFixture.apple)
    ...
    fun `should update a user cart`() {
        ...
        `when`(productsProvider.findBy(any(), anyString()))
