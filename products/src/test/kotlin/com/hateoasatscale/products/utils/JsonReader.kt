package com.hateoasatscale.users.utils
...
object JsonReader {
    private fun toJson(baseFolder: String, folder: String, testCase: String): String {
        ...
        requireNotNull(jsonURL) { "No JSON file found : $jsonFile" }
        try {
            Files.lines(Paths.get(jsonURL.toURI())).use { lines ->
                return lines.collect(Collectors.joining("")).filterNot { it.isWhitespace() }
            }
        } catch (e: Exception) {
